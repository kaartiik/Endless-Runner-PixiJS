...
export default class SmallDroid {
  constructor(posX, removeChild) {
    ...
    for (let i = 1; i <= 4; i++) {
      const texture = new PIXI.Texture.from(`droid_small_walking_${i}.png`);

      this.toonTextures[WALK].push(texture);
    }

    for (let i = 1; i <= 8; i++) {
      const texture = new PIXI.Texture.from(`droid_small_fall_${i}.png`);
    ...
    this.sprite.scale.x = this.isSpriteFlipped ? -0.5 : 0.5;
